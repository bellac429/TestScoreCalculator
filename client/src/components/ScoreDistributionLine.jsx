...
const ScoreDistributionLine = ({ medianScore, percentiles }) => {
    ...
    <div className="w-full max-w-3xl mx-auto p-4">
      <ResponsiveContainer width="80%" height={100}>
        <ScatterChart margin={{ top: 10, right: 20, left: 20, bottom: 30 }}>
          <XAxis type="number" dataKey="x" domain={percentiles} tick={{ fontSize: 15 }} label={{ value: "Score Distribution", position: "insideBottom", offset: -15 }} />
            
          <YAxis type="number" dataKey="y" hide /> {/* Hides the Y-axis as it's not needed */}
          <ReferenceDot x={medianScore} y={0} r={6} stroke="blue" strokeWidth={3} fill="blue" />
          <Scatter name="Median" data={[{ x: medianScore, y:0, size: 150 }]} fill="blue"/>
...
export default ScoreDistributionLine;
